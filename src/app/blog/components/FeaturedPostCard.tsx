...
export function FeaturedPostCard({ post, onCategoryClick }: FeaturedPostCardProps) {
  return (
    <Link href={`/blog/${post.slug}`} className="block group">
      <article className="group relative rounded-2xl overflow-hidden backdrop-blur-xl bg-card border border-border shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] cursor-pointer h-full flex flex-col">
